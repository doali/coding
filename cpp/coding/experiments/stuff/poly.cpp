...
public:
  ...
  virtual ~A() { std::cout << *this << " is about to be free" << std::endl; }
...
public:
  ...
  virtual ~B() { std::cout << *this << " subclass (being free)" << std::endl; }
  friend std::ostream &operator<<(std::ostream &os, const B &_b) {
    os << _b.label << " | " << _b.more;
    return os;
  }

  void doSomething() { std::cout << *this << " is running." << std::endl; }
};
